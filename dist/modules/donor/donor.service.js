...
let DonorService = class DonorService {
    ...
    async filterDonors() {
        try {
            ...
            const allDonors = await datasource_1.default.createQueryBuilder().select("donor").from(donor_entity_1.DonorEntity, "donor").where("dob like :dob", { dob: `%-${month}-${date}`.trim() }).getMany();
            return allDonors;
        }
        catch (e) {
            throw e;
